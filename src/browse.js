...
function ScrapeList(href, pageHtml) {
  ...
  if (content) {
    ...
    content.getElementByTagName('img').forEach(function(e, i) {
      var icon = e.attributes.getNamedItem('data-src');// || e.getElementByTagName('img')[0].attributes.getNamedItem('src');
      icon = (/^http.*/.test(icon.value) ? icon.value : BASE_URL + icon.value);
...
function showPlayersFolder(page, pageHtml) {
  try {
    ...
    //   i++;
    // }
    // document.getElementsByClassName('series-tab')
    pageHtml.dom.getElementByClassName('series-tab').forEach(function(tab, i) {
      title = tab.children[0].attributes.getNamedItem('data').value;
      page.appendPassiveItem('separator', null, {
        title: (title.match(/(\/\/(www\.)?([-a-zA-Z0-9@:%._\+~#=]{2,256}\.[a-z]{2,6}\b)[-a-zA-Z0-9@:%_\+.~#?&\/\/=]*)/) || [] )[3] || 'anidub',
      });
      console.log(tab.innerHTML);
      tab.children.forEach(function(item) {
        url = item.attributes.getNamedItem('data').value;
        console.log(url);
